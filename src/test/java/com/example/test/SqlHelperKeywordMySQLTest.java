...
/**
 * MySQL数据源SqlHelper关键字功能测试类
 * 测试SqlHelper支持的各种条件关键字和功能在MySQL数据源下是否正常工作
 */
@Slf4j
@SpringBootTest
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
public class SqlHelperKeywordMySQLTest {
    ...
    public void testNe() {
        ...
        assertNotNull(list);
        assertTrue(list.size() >= 1);
        list.forEach(user -> assertNotEquals("赵六", user.getName()));
    }
    ...
    public void testGt() {
        List<SysUserVO> list = SqlHelper.<SysUser>of()
                ...
                .list();
        assertNotNull(list);
        assertTrue(list.size() >= 1); // 赵六(28)、钱七(32)
        list.forEach(user -> assertTrue(user.getAge() > 27));
    }
    ...
    public void testGe() {
        List<SysUserVO> list = SqlHelper.<SysUser>of()
                ...
                .list();
        assertNotNull(list);
        assertTrue(list.size() >= 1); // 赵六(28)、钱七(32)、孙八(27)
        list.forEach(user -> assertTrue(user.getAge() >= 27));
    }
    ...
    public void testLt() {
        List<SysUserVO> list = SqlHelper.<SysUser>of()
                ...
                .list();
        assertNotNull(list);
        assertTrue(list.size() >= 1); // 赵六(28)、孙八(27)
    ...
    public void testLike() {
        ...
        assertNotNull(list);
        assertTrue(list.size() >= 1);
        assertTrue(list.get(0).getName().contains("赵"));
    }
    ...
    public void testNotLike() {
        ...
        assertNotNull(list);
        assertTrue(list.size() >= 1);
        list.forEach(user -> assertFalse(user.getName().contains("赵")));
    }
    ...
    public void testIn() {
        ...
        assertNotNull(list);
        assertTrue(list.size() >= 1);
        Set<Integer> ages = new HashSet<>(Arrays.asList(28, 32));
    ...
    public void testIsNull() {
        ...
        user.setName(null);
        user.setAge(42);
        user.setBirthDate(LocalDate.of(1981, 1, 1));
        user.setState(1);
        user.setCreateTime(LocalDateTime.now());
        user.setUpdateTime(LocalDateTime.now());
        user.setDeleted(0);
        user.setVersion(1);
        sysUserDynamicFieldService.save(user);
        ...
        // 清理临时数据
        sysUserDynamicFieldService.removeById(user.getId());
    }
    ...
    public void testOr() {
        ...
        assertNotNull(list);
        assertTrue(list.size() >= 1);
        Set<String> names = new HashSet<>(Arrays.asList("赵六", "钱七"));
